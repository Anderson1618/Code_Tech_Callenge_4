...
fig = px.line(forecast, x='ds', y=['yhat', 'yhat_lower', 'yhat_upper'], title='Previsão do Preço do Petróleo (Brent) para os Próximos 6 meses', 
              color_discrete_map={'yhat': 'green', 'yhat_lower': 'red', 'yhat_upper': 'blue'})
fig.add_scatter(x=df_prophet['ds'], y=df_prophet['y'], mode='lines', name='Observado', line=dict(color='black'))
fig.update_xaxes(title_text='Data')
